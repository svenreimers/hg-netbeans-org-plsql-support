...
/**
 * Check whether the method name is missing at the end of a method
 * @author YADHLK
 */
public class PlsqlMissingEndNameAnnotation extends PlsqlBlockAnnotation {

   private String methodName;

   public PlsqlMissingEndNameAnnotation(String methodName, int offset, int ignoreOffset) {
   ...
   public static PlsqlMissingEndNameAnnotation getDummyInstance() {
      return new PlsqlMissingEndNameAnnotation();
   }

   private PlsqlMissingEndNameAnnotation() {
   } //Used for configuration only

   @Override
   public String getErrorToolTip() {
      return NbBundle.getMessage(this.getClass(), "missing_end_name_annotation");
   }

   @Override
   public String getAnnotationType() {
      return "Plsql-missing-end-name-annotation";
   }
      ...
      if (ts != null) {
         ...
         while (ts.movePrevious() && ts.offset() > block.getStartOffset()) {
            ...
            if (token.toString().equalsIgnoreCase("END")) {
               if (!isName && ts.moveNext()) {
                  exists = true;
                  PlsqlMissingEndNameAnnotation endNameAnnotation = new PlsqlMissingEndNameAnnotation(block.getName(), ts.offset(), block.getStartOffset());
   ...
   public Action[] getActions() {
      if (isIgnoreAlowed()) {
         return new Action[]{new AddNameAction(), new AddIgnoreMarkerAction()};
      } else {
         return new Action[]{new AddNameAction()};
      }
   }
   ...
   public String getIgnoreKey() {
      return "MissingEndName";
   }

   private class AddNameAction extends AbstractAction {

      @Override
      public String toString() {
         return "Add the function/procedure name";
      }
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         if (annotationManager != null) {
            try {
               doc.insertString(offset, " " + methodName, null);
