 ...
 * @author ChrLSE
 */
public class PlsqlViewTarget extends AbstractPlsqlGotoTarget {

   public PlsqlViewTarget(final String name, final int position, final DataObject sourceDataObject,
           final Project project, final Document sourceDocument) {
      super(name, position, sourceDataObject, project, sourceDocument);
      this.type = PlsqlBlockType.VIEW;
      this.synonym = cache.getViewForSynonym(name);
   }
   ...
   public void gotoBody() {
      ...
      try {
         targetDataObject = PlsqlFileUtil.openExistingFile(sourceDocument, getName(), type, project);
         if (targetDataObject == null) {
            PlsqlHyperlinkUtil.openAsTempFile(getName(), type, connection, project, sourceDocument);
         } else {
            PlsqlHyperlinkUtil.setCaretOfView(targetDataObject, getName());
         }
